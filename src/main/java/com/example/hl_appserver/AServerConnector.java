...
public class AServerConnector{
	...
	public void onOpen(Session session, EndpointConfig ec){
		//セッションを記憶
		boolean room1; establishedSessions.add(session);
		//ログに出力
	...
	public void onMessage(final String message, final Session session) throws IOException{
		...
		if(receivedMessage.order.equals("1000")){
			List<Integer> scoreList; Message message5000 = new Message("5000", receivedMessage.messageContent.user_id);
			...
			message5000.messageContent.num_hits_score = scoreList.get(2); sendMessage(session, message5000);
		}else if(receivedMessage.order.equals("1001")){
			Message message5001 = new Message("5001", receivedMessage.messageContent.user_id);
			message5001.messageContent.textDataList.add(getRule()); sendMessage(session, message5001);
		}else{
		}
	}
	...
	public void onClose(Session session){
		System.out.println("[WebSocketServerExample] onClose:" + session.getId()); establishedSessions.remove(session);
	...
	public void sendMessage(Session session, Message message){
		...
		String send_message = gson.toJson(message); try{
			// 同期送信（sync）
	...
	public String getRule(){
		String rule; rule = databaseConnector.getRule(); return rule;
	}

	public List<Integer> getScore(String user_id){
		List<Integer> scoreList; scoreList = databaseConnector.getScore(user_id); return scoreList;
	}
